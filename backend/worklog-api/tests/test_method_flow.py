...
class TestMethodFlow:
    def test_method_flow_http_entry_uses_only_named_stages(self) -> None:
        source = SOURCE_ROOT / "worklog/adapters/http/activity_endpoint.py"
        assert (
            validate_method_flow(
                source.read_text(), "ActivityEndpoint", "post_activity"
            )
            == []
        )

    def test_method_flow_usecase_uses_only_named_stages(self) -> None:
        source = SOURCE_ROOT / "worklog/application/start_activity.py"
        assert validate_method_flow(source.read_text(), "StartActivity", "run") == []

    def test_method_flow_application_factory_uses_only_named_stages(self) -> None:
    ...
    def test_composition_root_has_no_module_level_functions(self) -> None:
        source = SOURCE_ROOT / "worklog/main.py"
    ...
    def test_method_flow_accepts_methods_on_the_responsible_object(self) -> None:
        source = """
        class Flow:
            def run(self):
                value = self.read()
    ...
    def test_method_flow_rejects_free_function_call(self) -> None:
        source = """
        class Flow:
            def run(self):
                value = self.read()
    ...
    def test_method_flow_rejects_low_level_package_call(self) -> None:
        source = """
        class Flow:
            def run(self):
                value = self.read()
